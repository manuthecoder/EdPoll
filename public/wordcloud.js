  ...
  layout
    ...
    layout
      ...
      .words(
        tags.slice(
          0,
          (max = Math.min(tags.length, +max_value))
        )
      )
      .start();
}
...
form.selectAll("input[type=number]").on("click.refresh", function () {
  ...
  form.selectAll("input[type=radio]").on("change", generate);
var stopWords =
...
d3.select("#random-palette").on("click", function () {
  ...
  (function () {
    function t() {
			c = + 2, u = Math.max(-90, Math.min(90, +angleFROM)), i = Math.max(-90, Math.min(90, +angleTO)), e()
